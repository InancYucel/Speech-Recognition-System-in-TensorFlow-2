...
# İş boyutu. Ağın bir update'den önce göreceği örnek sayısı belirtir ve geri yayımlım algoritması gibi çalışır. Learning SAP gibi çalışır.
BATCH_SIZE = 32
...
def get_data_splits(data_path, test_size=0.1, test_validation=0.1):
    ...
    # test_size şu demek veri kümesinden test kümesine katılacak olanların oranı
    # train_test_split fonksiyonunu input değelerini bu şekil "X_train, X_test, y_train, y_test" output değerlerine ayırmak için kullanıyoruz.
    # Temelde veri setinin %90'ı eğitim ve doğrulamaya gidiyor

    # train validation ve test splits girdilerini görmemiz gerekiyor. İki boyutlu diziler olacak çünkü bunlar JSON dosyasından geliyor. Temelde bunlar MFCC değerleri
    # bu yüzden burada iki boyutlu dizileri, serbest boyutlu dizilere dönüştürmemiz gerekiyor.
...
def build_model(input_shape, learning_rate):
    ...
    # build network
    # sequential - ardışık bir model oluşturmak istiyoruz. Bir sürü ardışık katmana sahip bir ayna ağıdır. Biri diğerine, yenisi üçüncüsüne etc
    ...
    # overfitting'i ele almak için başka bir teknik.Dropout rate - Bırakma oranı.
    # Eğitim sırasında dense layerdaki neuronların %30'unu kapatmak. Bu fonksiyonu AI'a sor.
    # Bunu yaparsak anlyıoruz ki ağın sınıflandırma yapmak için mutlaka bir veya birkaç nörona güvenmemesi,
    # bunun yerine ağın uyum sağlaması ve tüm farklı neuronların sınıflandırmanın kendisi için eşit sorumluluk alması gerekliliğidir.
    model.add(keras.layers.Dropout(0.3))

    # softmax classifier

def main():
    print("Available GPUs:", keras.config.list_physical_devices('GPU'))
    print("Using device:", "GPU" if keras.config.list_physical_devices('GPU') else "CPU")
